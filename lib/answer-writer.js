...
const ATTESTATION_RE = new RegExp(
  [
    ...
    // Who the candidate knows, and how they found the posting. Neither is in a
    // resume; the model declined both honestly, but declining cost a call each
    // time and read as a failure rather than a routing decision.
    'know anyone', 'anyone (currently )?(at|who works)', 'referred by', 'referral',
    ...
    // Working-arrangement commitments. CLAUDE.md names these explicitly: "Are
    // you located in the NYC area?", "willing to come in 3 days/week?" are real
    // decisions about how the candidate will live, not prose about past work.
    // They read as ordinary questions and would otherwise be written as
    // narrative, so they are listed here to stop for the human.
    'hybrid policy', 'in-?office', 'onsite', 'on-site', 'days (per|a) week',
    'commute', 'willing and able to commit', 'able to commit to',
    'work from (the )?office', 'report to (the|an) office',
  ].join('|'),
  'i'
);
...
function groundingViolations(text, allowed) {
  ...
  for (const sentence of String(text).split(/(?<=[.!?:])\s+|\n+/)) {
    // Split on slashes and hyphens as well as whitespace. "LLM/agent" and
    // "LLM-driven" are single whitespace tokens, and testing them whole rejected
    // perfectly grounded answers whose halves both appear in the facts file — a
    // false accusation of fabrication that varied run to run with the model's
    // phrasing. Splitting cannot hide a fabrication: "Google-scale" still yields
    // "Google", which is still caught.
