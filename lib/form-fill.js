...
const { ATTESTATION_RE } = require('./answer-writer');

// Voluntary demographic data and anything else the agent must never choose.
// Voluntary demographic self-identification, and nothing else.
//
// Salary used to be here, from when the facts file had no answer for it. It does
// now — screening_answers.desired_salary, written by the human — so blocking it
// here meant refusing to use an answer they had explicitly provided, and every
// Greenhouse form asking for compensation stalled on it. It is matched in
// SCREENING_MAP below like any other attestation and reused verbatim.
const NEVER_FILL =
  /pronoun|gender|sex\b|race|ethnic|hispanic|latino|veteran|disab|lgbt|orientation|age\b|birth/i;
...
const FIELD_MAP = [
  ...
  // A real city resolves: the list offers "<City>, <State>, USA".
  ...
  // Country BEFORE the generic location rule. "What country are you based in?"
  // matches "based in" too, and the location rule answered it with the resume
  // header "San Francisco Bay Area, CA" — a metro area offered to a dropdown of
  // countries, which matched nothing and stalled the application.
  ...
  { re: /\b(location|based in|currently based|where are you based|current residence)\b/i,
    get: (f) => f.contact.location },
  ...
  { re: /most recent degree|degree you (obtained|earned|received)|highest degree|highest level of education|education level/i,
    get: (f) => (f.education || [])[0] && f.education[0].credential, beforeAttestation: true },

  // Graduation year and GPA.
  //
  // beforeAttestation: these look like attestations ("graduation date", "gpa")
  // and ATTESTATION_RE lists both, so without the flag they stopped for the
  // human. But they are FACTS the human already wrote into master-facts.json —
  // the same data generate.js prints on the resume — not inferences. The
  // attestation list exists to stop the model guessing what it cannot know;
  // it should not block a value the candidate supplied.
  { re: /graduat\w*\s*(year|date)|year .{0,15}graduat|when did you graduate/i,
    get: (f) => {
      const e = (f.education || [])[0];
      const end = e && String(e.end || '');
      return /^\d{4}/.test(end) ? end.slice(0, 4) : null;
    }, beforeAttestation: true },
  { re: /\bgpa\b|grade point average/i,
    get: (f) => {
      const e = (f.education || [])[0];
      if (!e || !e.gpa) return null;
      // "3.4 / 4.0" -> "3.4"; a form asking "on a 4.0 scale" wants the number.
      return String(e.gpa).split('/')[0].trim();
    }, beforeAttestation: true },
];
...
// the candidate's stored US work-authorisation answer. Nothing was
...
const SCREENING_MAP = [
  ...
  // "eligible to work" was missing, so "Are you currently eligible to work in
  // the country where the role is based?" fell past every attestation rule and
  // was answered by the FIELD_MAP country rule with the bare string "United
  // States" — a country name offered to a yes/no work-authorisation question.
  { re: /legally authorized|authorized to work|work authorization|right to work|eligible to work|authoriz\w* for employment/i,
  ...
  // Also US-only: the stored answer is specific to the United States.
  ...
  // Compensation. A number stated to an employer is a negotiating position and
  // an attestation, so it is the human's — but they HAVE written one, and not
  // using it stalls most Greenhouse forms on a question already answered.
  { re: /compensation expectation|salary expectation|expected (salary|compensation)|desired (salary|compensation)|salary requirement|compensation requirement|what .{0,25}(salary|compensation)|(salary|compensation) .{0,15}(range|expectation)/i,
    key: 'desired_salary' },
  // How the posting was found. Deterministic, not a guess: discover.js polls the
  // company's own public ATS board API, so "company job board" is a description
  // of what the pipeline actually did.
  { re: /how did you (hear|find out|learn)|where did you (hear|find)|how were you referred|source of (this )?application/i,
...
function valueFor(facts, label, extra) {
  ...
  // An attestation that no stored answer covers must NOT fall through to
  // FIELD_MAP. GitLab asks "Are you subject to any employment agreements and/or
  // post-employment restrictions?" — which matches /current or previous
  // employer/ and was answered with the candidate's employer name, "Metis AI
  // Inc.", against a non-compete question. Same shape as the South Korea bug:
  // an attestation losing to a contact-shaped match. FIELD_MAP is for facts
  // about the candidate; a question the attestation list recognises is never one
  // of those, so it stops here for the human instead.
  // Fact-derived fields that the attestation list would otherwise swallow.
  // Checked BEFORE the gate, and only these — see beforeAttestation above.
  for (const m of FIELD_MAP) {
    if (!m.beforeAttestation) continue;
    if (m.re.test(hay)) {
      const v = m.get(facts);
      if (v) return { value: v, source: 'education' };
    }
  }

  if (ATTESTATION_RE.test(hay)) {
    return { needsHuman: 'unmapped attestation', unmapped: true };
  }
...
async function attachResume(page, resumePath, out) {
  ...
  for (const el of inputs) {
    const ctx = await el.evaluate((e) => {
      ...
      // id and name FIRST. GitLab's Greenhouse embed renders both slots as
      // class="visually-hidden" with no label and no aria-label, and the nearest
      // ancestor carrying text says only "Attach / Enter manually / Accepted
      // file types" — which names neither resume nor cover letter. The ids are
      // literally `resume` and `cover_letter`, so ignoring them meant the resume
      // slot could not be identified on one of the biggest boards we target.
      return `${e.id || ''} ${e.name || ''} ${own} ${up}`.toLowerCase();
    });
    const id = await el.evaluate((e) => e.id || '');
    described.push({ el, ctx, id });
  }
  ...
  if (target) {
    ...
    for (let attempt = 1; attempt <= 3 && !attached; attempt++) {
      // The error is CAPTURED, not swallowed. Greenhouse replaces the input
      // element after an upload, so a handle taken before the attach can be
      // detached by the retry — and a silent catch here reports "the board would
      // not accept the upload" for what is actually a stale element handle.
      await target.el.setInputFiles(resumePath).catch((e) => { lastErr = e; });
      // 25s, not 12s. The upload is an async round trip to S3, and on a 4-core
      // box running other pipeline stages concurrently the render lagged past
      // 12s often enough that 5 of 37 jobs in one sweep reported "the board
      // would not accept the upload" — all 5 attached on a retry with nothing
      // else running. A slow box is not a rejected file.
      for (let waited = 0; waited < 25000; waited += 750) {
      ...
      if (!attached && attempt < 3) {
        ...
        // Re-resolve the input by id before retrying: the handle may be stale.
        const fresh = await page.$(`input[type=file]#${target.id || 'resume'}`).catch(() => null);
        if (fresh) target.el = fresh;
      }
    }
    if (attached) out.filled.push(`resume <- ${base}`);
    else {
      out.unanswered.push(
        `resume (board would not accept the upload after 3 attempts${lastErr ? `: ${lastErr.message.split('\n')[0]}` : ''})`
      );
    }
  }
...
const COMBO_SYNONYMS = [
  ...
  ['yes', 'y', 'true'],
  ['no', 'n', 'false'],
  // How the posting was found. Boards word this option a dozen ways; they all
  // mean the same channel, which is the one discover.js actually used.
  ['company website / job board', 'company website', 'job board', 'company site',
   'careers page', 'careers site', 'company careers page', 'job boards',
   'company job board', 'website', 'other job board'],
];
...
function comboCandidates(want) {
  ...
  // Drop a trailing qualifier: "Springfield, IL" -> "Springfield". A geocoded list
  // returns "Springfield, IL, USA", which the bare city name prefix-matches while
  ...
  // The bare Yes/No a sentence answer opens with.
  //
  // screening_answers stores the human's full sentence — "Yes — will require
  // H-1B sponsorship in the future." — because that is what they wrote and it is
  // reused verbatim. A <select> already had this fallback; a react-select combo
  // did not, so every sponsorship question on a Greenhouse embed stalled on an
  // option list of exactly ["Yes", "No"]. Still the human's answer, matched to
  // the option the form offers, and the leading token is unambiguous.
  const yn = /^\s*(yes|no)\b/i.exec(w);
  if (yn) out.push(yn[1][0].toUpperCase() + yn[1].slice(1).toLowerCase());
...
function comboScore(optionText, want) {
  ...
  //
  // The length floor is not cosmetic. GitLab's sponsorship dropdown is a GLOBAL
  // visa list, and the stored answer "Yes — will require H-1B sponsorship in the
  // future." reduces to the candidate "Yes", which prefix-matched
  // "Yes, Netherlands Highly Skilled Migrant Visa" and scored 80. That is a
  // false immigration attestation naming a country the candidate has no status
  // in — the exact failure the never-guess rule exists to prevent, arrived at
  // through a fuzzy match rather than a guess. A short needle must match an
  // option EXACTLY (o === w above, score 100) or not at all.
  if (w.length >= 4 && (o.startsWith(w) || w.startsWith(o))) return 80;
...
/**
 * Open-ended questions on the page that no stored answer covers.
 *
 * Run BEFORE fillForm so the answer-writer can be called once per job rather
 * than once per question. Only textareas are surveyed: a <select> or radio
 * offers a fixed set of options, and choosing one the matcher could not match
 * is guessing at a structured claim rather than writing prose.
 */

// A question-shaped label on a short-answer control. Greenhouse renders custom
// questions as input[type=text], NOT textarea — "Have you built AI agents? If
// so, please elaborate." is a text input, so a textarea-only survey missed
// every one of them and the application stalled on a required field the facts
// file could answer.
const NARRATIVE_SHAPE =
  /\?\s*\*?\s*$|describe|elaborate|tell us|walk us through|why (do|are|would)|what (interests|excites|draws|motivates)|share (an?|your)|example of/i;

async function surveyOpenQuestions(page, facts) {
  const asks = [];
  for (const el of await page.$$('textarea, input[type=text]')) {
    const info = await describe(el);
    if (info.type === 'hidden') continue;
    const label = (info.label || '').trim();
    if (!label || NEVER_FILL.test(label)) continue;
    if (valueFor(facts, label, info.name)) continue; // already answerable
    // Text inputs must LOOK like an open question. A textarea is one by
    // construction; a text input is usually a name or a URL, and writing prose
    // into one of those is worse than leaving it empty.
    if (info.tag !== 'textarea' && !NARRATIVE_SHAPE.test(label)) continue;
    const already = await el.evaluate((e) => (e.value || '').trim().length);
    if (already) continue;
    const maxLength = await el.evaluate((e) => (e.maxLength > 0 ? e.maxLength : null));
    asks.push({ label, tag: 'textarea', maxLength, required: info.required });
  }
  return asks;
}
 ...
 *
 * `generated` is an optional Map of question label -> { answer, factIds } from
 * lib/answer-writer.js. It is consulted ONLY where no stored answer matched and
 * only for textareas, so it can never override or stand in for an attestation.
 */
async function fillForm(page, facts, resumePath, { generated } = {}) {
  const out = { filled: [], skipped: [], unanswered: [], generated: [] };
  ...
  for (const el of await page.$$('input, textarea, select')) {
    ...
    if (hit && hit.needsHuman) {
      out.unanswered.push(hit.wrongCountry
        ...
        : hit.unmapped
          ? `${tag} (attestation with no stored answer — answer it yourself)`
          : `${tag} (attestation, no answer in screening_answers.${hit.needsHuman})`);
      continue;
    }
    if (!hit) {
      // A model-written narrative answer, if one was generated for this exact
      // question. Textareas only, and only where nothing stored matched — an
      // attestation always matches SCREENING_MAP first and returns needsHuman
      // above, so this branch is unreachable for one.
      const gen = generated && generated.get(info.label);
      // Same gate the survey used: a textarea, or a text input whose label is
      // shaped like an open question. Anything else stays empty.
      const freeText = info.tag === 'textarea' ||
        (info.tag === 'input' && NARRATIVE_SHAPE.test(info.label || ''));
      if (gen && freeText) {
        let ok = true;
        await el.fill(gen.answer).catch(() => { ok = false; });
        if (ok) {
          out.filled.push(`${tag} = [written from facts ${(gen.factIds || []).join(', ') || '—'}]`);
          out.generated.push({ question: info.label, answer: gen.answer, factIds: gen.factIds || [] });
          continue;
        }
      }
    ...
    try {
      if (info.tag === 'select') {
        ...
        if (!ok) {
          // A sentence answer ("Yes - ...") against options [Yes, No]:
...
module.exports = {
  fillForm, auditRequired, applyUrlFor, surveyOpenQuestions, NEVER_FILL, SCREENING_MAP,
};
