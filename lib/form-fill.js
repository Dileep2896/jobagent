...
async function fillCombobox(page, el, want) {
  ...
  for (const cand of candidates) {
    ...
    const got = rank(await readOptions());
...
async function fillForm(page, facts, resumePath) {
  ...
  for (const el of await page.$$('input, textarea, select')) {
    ...
    if (['checkbox', 'radio'].includes(info.type)) {
      ...
      // do you anticipate working". The stored answer names an option directly,
      // so tick the one it names. Exact, case-insensitive match only — a fuzzy
      // match here would tick a country the human never chose.
      if (answer && info.label.trim() &&
          String(answer).toLowerCase().trim() === info.label.trim().toLowerCase()) {
        try {
          await el.check();
          answeredGroups.add(groupKey);
          out.filled.push(`${q} = ${info.label.trim()} <- screening_answers.${match.key}`);
