...
async function describe(el) {
  return el.evaluate((e) => ({
    ...
    label: (() => {
      ...
      return (e.placeholder || '').replace(/\s+/g, ' ').trim();
    })(),
...
function comboCandidates(want) {
  ...
  // "San Francisco Bay Area, CA" -> "San Francisco", which is what a geocoded
  // city list actually offers.
  const words = w.replace(/,.*$/, '').trim().split(/\s+/);
